...
namespace THOK.SMS.Bll.Service
{
    public class SmsDeviceFaultService : ServiceBase<SmsDeviceFault>, ISmsDeviceFaultService
    {
        ...
        public object GetDetails(int page, int rows, string DeviceCode, string DeviceType, string FaultCode, string BeginTime, string EndTime, string UseTime)
        {
            ...
            var v1 = smsDeviceFaultDetail6.OrderBy(a => a.Id).Select(t => t);
            int total = v1.Count();
            v1 = v1.Skip((page - 1) * rows).Take(rows);

            var smsDeviceFaultsArray = v1.ToArray().Select(s => new
                {
                    s.Id,
                    s.DeviceCode,
                    s.DeviceName,
            ...
            return new { total, rows = smsDeviceFaultsArray };
        }

        public DataTable GetSmsDeviceFault(int page, int rows, string DeviceCode, string DeviceType, string FaultCode, string BeginTime, string EndTime, string UseTime)
        {
            IQueryable<SmsDeviceFault> smsDeviceFaultQuery = SmsDeviceFaultRepository.GetQueryable();
            IQueryable<SmsDeviceFault> smsDeviceFaultDetail1 = smsDeviceFaultQuery;
            if (DeviceCode != "")
            ...
            var v1 = smsDeviceFaultDetail6.OrderBy(a => a.Id).Select(t => t);
            var smsDeviceFaultsArray = v1.OrderBy(s => s.Id).AsEnumerable()
                .Select(s => new
                {
                    s.Id,
                    s.DeviceCode,
