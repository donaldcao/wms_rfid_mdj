...
namespace THOK.SMS.Optimize.Service
{
    public class DeliverLineOptimizeService : ServiceBase<SortOrder>, IDeliverLineOptimizeService
    {
        ...
        private string[] GetDeliverLineAllotState(string deliverLineCode)
        {
            ...
            //var deliverLineState = deliverLineAllotQuery.Where(a => a.DeliverLineCode.Contains(deliverLineCode)).Select(a => new { a.Status, a.sortBatch.SortingLineCode }).ToArray();
        ...
        public bool UpdateDeliverLineAllot(string orderDate, string deliverLineCodes, string userName, out string strResult)
        {
            ...
            //        }
            //        //更新配送线路分配表
            //        IQueryable<SortBatch> sortBatchQuery = SortBatchRepository.GetQueryable();
            //        var sortBatchDetails = sortBatchQuery.Where(a => a.BatchId.Equals(batchId)).Select(a => new { a.SortBatchId, a.SortingLineCode }).ToArray();
            //        IQueryable<SortOrder> sortOrderQuery = SortOrderRepository.GetQueryable();
            //        var sortOrderDetails = sortOrderQuery.Where(s => s.OrderDate.Equals(orderDate))
            ...
            //        {
            //            int sortBatchId = sortBatchDetails.FirstOrDefault(a => a.SortingLineCode.Equals(sortOrderDispatchDetails.FirstOrDefault(b => b.DeliverLineCode.Equals(item)).SortingLineCode)).SortBatchId;
            //            DeliverLineAllot deliverLineAllot = new DeliverLineAllot();
            //            deliverLineAllot.DeliverLineAllotCode = sortBatchId.ToString() + "-" + item;
            //            deliverLineAllot.SortBatchId = sortBatchId;
        ...
        public bool UpdateDeliverLineAllot(string orderDate, string deliverLineCodes, string sortingLineCode, string userName, out string strResult)
        {
            ...
            //    {
            //        var sortBatchId = sortBatchDetails
            //            .Where(a => sortingLineCodeArray.Contains(a.SortingLineCode))
            //            .Select(a => new
            //            {
            //                a.SortBatchId,
            //                Quantity = deliverLineAllotQuery.GroupBy(b => b.SortBatchId).Select(b => new { b.Key, Quantity = b.Sum(c => c.DeliverQuantity) }).ToArray().FirstOrDefault(b => b.Key.Equals(a.SortBatchId)) == null ?
            //                           0 : deliverLineAllotQuery.GroupBy(b => b.SortBatchId).Select(b => new { b.Key, Quantity = b.Sum(c => c.DeliverQuantity) }).ToArray().FirstOrDefault(b => b.Key.Equals(a.SortBatchId)).Quantity
            //            }).ToArray().OrderBy(a => a.Quantity).FirstOrDefault().SortBatchId;
            //        DeliverLineAllot deliverLineAllot = new DeliverLineAllot();
            //        deliverLineAllot.DeliverLineAllotCode = sortBatchId.ToString() + "-" + sortOrderDetails[i].DeliverLineCode;
            //        deliverLineAllot.SortBatchId = sortBatchId;
        ...
        private int GetBatchId(DateTime date, string orderDate, string userName, Array sortingLineCodes, out string strResult)
        {
            ...
            //{
            //    var sortBatchDetails = sortBatchQuery.AsEnumerable().FirstOrDefault(a => a.BatchId.Equals(batchId) && a.SortingLineCode.Equals(item.ToString()));
            //    if (sortBatchDetails == null)
            //    {
            //        SortBatch sortBatch = new SortBatch();
            //        sortBatch.BatchId = batchId;
            //        sortBatch.SortingLineCode = item.ToString();
            //        sortBatch.Status = "01";
            //        bool result = SortBatchService.Add(sortBatch, out strResult);
            //        if (!result)
            //        {
            //            return -1;
            //        }
