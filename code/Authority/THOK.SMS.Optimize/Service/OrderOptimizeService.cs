...
namespace THOK.SMS.Optimize.Service
{
    public class OrderOptimizeService : ServiceBase<SortOrder>, IOrderOptimizeService
    {
        ...
        {
            var batchSortQuery = BatchSortRepository.GetQueryable();
            ...
            var batchSortIds = batchSortQuery.Where(a => a.batch.OrderDate == date)
                                           .Select(b => b.BatchSortId)
                                           .Distinct()
                                           .ToArray();

            //遍历每条批次分拣线
            foreach (int batchSortId in batchSortIds)
            {
                var deliverLines = deliverLineAllot.Where(a => a.BatchSortId == batchSortId)
                ...
                var channelAllots = channelAllotQuery.Where(c => c.BatchSortId == batchSortId)
                                                     .ToArray();
                ...
                foreach (var sortOrder in sortOrders)
                {
                    ...
                    foreach (var item in BagQuantity)
                    {
                        ...
                        addSortOrderAllotMaster.BatchSortId = batchSortId;
                        ...
                        addSortOrderAllotMaster.OrderMasterCode = batchSortId + "-" + item.Key;
                    ...
                    //            addSortOrderAllotDetail.OrderMasterCode = batchSortId + "-" + detailPackNo;
                    //            addSortOrderAllotDetail.OrderDetailCode = batchSortId + "-" + detailPackNo + "-" + channelAllotCode;
