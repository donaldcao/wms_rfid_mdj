...
namespace THOK.WCS.Bll.Service
{
    public class WcsDeviceStateService : ServiceBase<WcsDeviceState>, IWcsDeviceStateService
    {
        ...
        public object GetDetails(int page, int rows, string DeviceCode, string DeviceType, string StateCode, string BeginTime, string EndTime, string UseTime)
        {
            ...
            var v1 = wcsDeviceStateDetail6.OrderBy(a => a.Id).Select(t => t);
            int total = v1.Count();
            v1 = v1.Skip((page - 1) * rows).Take(rows);

            var wcsDeviceStatesArray = v1.ToArray().Select(s => new
                {
                    s.Id,
                    s.DeviceCode,
                    s.DeviceName,
            ...
            return new { total, rows = wcsDeviceStatesArray };
        }

        public DataTable GetWcsDeviceState(int page, int rows, string DeviceCode, string DeviceType, string StateCode, string BeginTime, string EndTime, string UseTime)
        {
            IQueryable<WcsDeviceState> wcsDeviceStateQuery = WcsDeviceStateRepository.GetQueryable();
            IQueryable<WcsDeviceState> wcsDeviceStateDetail1 = wcsDeviceStateQuery;
            if (DeviceCode != "")
            ...
            var v1 = wcsDeviceStateDetail6.OrderBy(a => a.Id).Select(t => t);
            var wcsDeviceStatesArray = v1.OrderBy(s => s.Id).AsEnumerable()
                .Select(s => new
                {
                    s.Id,
                    s.DeviceCode,
