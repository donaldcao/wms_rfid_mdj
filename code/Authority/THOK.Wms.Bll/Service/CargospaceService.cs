    ...
    public class CargospaceService : ServiceBase<Storage>, ICargospaceService
    {
        ...
        public object GetCellDetails(int page, int rows, string type, string id)
        {
            try
            {
                ...
                var temp = storages.OrderBy(s => s.CellCode).ThenBy(s => s.StorageSequence).Where(s => s.Quantity > 0);
        ...
        public object GetInCellDetail(string type, string id)
        {
            try
            {
                IQueryable<Cell> cellQuery = CellRepository.GetQueryable();
                IQueryable<Storage> storageQuery = StorageRepository.GetQueryable()
                    .Where(s => s.InFrozenQuantity == 0 
                        && s.OutFrozenQuantity == 0 
                        && s.Cell.IsActive == "1" 
                        && s.IsActive == "1"
                        && (s.LockTag == "" || s.LockTag == null));
                ...
                if (type == "ware")
                {
                    cells = cellQuery.Where(c => c.Shelf.Area.Warehouse.WarehouseCode == id && c.IsActive != "0");
                }
                else if (type == "area")
                {
                    cells = cellQuery.Where(c => c.Shelf.Area.AreaCode == id && c.IsActive != "0");
                }
                else if (type == "shelf")
                {
                    cells = cellQuery.Where(c => c.Shelf.ShelfCode == id && c.IsActive != "0");
                }
                else if (type == "cell")
                {
                    cells = cellQuery.Where(c => c.CellCode == id && c.IsActive != "0");
                }

                var sCells = cells.Join(storages, c => c.CellCode, s => s.CellCode, (c, s) => new { cell = c, storage = s })
                                  .ToArray();
                ...
                if (sCells.Count() > 0)
                {
                    var Cell = sCells.Select(c => new
                    {
                        cellCode = c.cell.CellCode,
                        cellName = c.cell.CellName,
                        storageCode = c.storage.StorageCode,
                        productCode = string.IsNullOrEmpty(c.storage.ProductCode) == true ? "" : c.storage.ProductCode,
                        productName = c.storage.Product == null ? "" : c.storage.Product.ProductName,
                        shelfCode = c.cell.ShelfCode,
                        unitCode = c.storage.Product == null ? "" : c.storage.Product.Unit.UnitCode,
                        unitName = c.storage.Product == null ? "" : c.storage.Product.Unit.UnitName,
                        maxQuantity = c.cell.MaxQuantity,
                        asdd1 = c.cell.AreaCode,//c.cells.CellCode.Substring(4, 2),//区
                        asdd2 = c.cell.ShelfCode,//c.cells.CellCode.Substring(8, 2),//货架
                        asdd3 = c.cell.Layer,//c.cells.CellCode.Substring(14, 1),//层
                        asdd4 = c.cell.Col,//c.cells.CellCode.Substring(11, 2),//列
                        Quantity = c.storage.Product == null ? 0 : c.storage.Quantity / c.storage.Product.Unit.Count,
                        EmptyQuantity = c.storage.Product == null ? c.cell.MaxQuantity : c.cell.MaxQuantity - c.storage.Quantity / c.storage.Product.Unit.Count,
                        IsActive = c.cell.IsActive == "1" ? "可用" : "不可用",
                        UpdateTime = c.storage.UpdateTime.ToString("yyyy-MM-dd"),
                        StorageSequence = c.storage.StorageSequence 
                    })
                    .OrderByDescending(c => c.asdd1)
                    .ThenByDescending(c => c.asdd2)
                    .ThenBy(c => c.asdd3)
                    .ThenBy(c => c.asdd4)
                    .ThenByDescending(c => c.StorageSequence);
            ...
            try
            {
                ...
                IQueryable<Storage> storageQuery = StorageRepository.GetQueryable()
                    .Where(s => s.InFrozenQuantity == 0 
                        ...
                        && (s.LockTag == "" || s.LockTag == null)
                        && s.Quantity > 0 
                        && s.ProductCode.Contains(productCode));
