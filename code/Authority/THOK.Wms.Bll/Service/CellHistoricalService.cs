        ...
        public object GetCellDetails(int page, int rows, string beginDate, string endDate, string type, string id)
        {
            ...
            if (type == "cell")
            {
                var CellHistoryAllQuery = InBillAllotQuery.Where(i => i.Status == "2" && i.CellCode == id)
                    ...
                    .Concat(OutBillAllotQuery.Where(o => o.Status == "2" && o.CellCode == id)
                        .Select(o => new
                        {
                            o.CellCode,
                            o.Cell.CellName,
                    ...
                    .Concat(MoveBillDetailQuery.Where(m => m.Status == "2" && (m.InCellCode == id || m.OutCellCode == id))
                        .Select(m => new 
                        {
                    ...
                    .Concat(ProfitLossBillDetailQuery.Where(p => p.ProfitLossBillMaster.Status == "3" && p.CellCode == id)
                        .Select(p => new
                        {
                            p.CellCode,
                            ...
                            p.Unit.Count,
                            IfMove = "0",
                            FinishTime = p.ProfitLossBillMaster.VerifyDate 
                        }));

                //null02 同null01 代表传回来的日期是空值
                if (!beginDate.Equals(string.Empty) && beginDate != "null01")
                {
                    DateTime begin = Convert.ToDateTime(beginDate);
                    CellHistoryAllQuery = CellHistoryAllQuery.Where(c => c.FinishTime >= begin);
                }

                if (!endDate.Equals(string.Empty) && endDate != "null02")
                {
                    DateTime end = Convert.ToDateTime(endDate);
                    CellHistoryAllQuery = CellHistoryAllQuery.Where(c => c.FinishTime <= end);
                }

                CellHistoryAllQuery = CellHistoryAllQuery.Where(c => c.RealQuantity > 0 && c.FinishTime != null)
                    .OrderByDescending(c => c.FinishTime);
                int total = CellHistoryAllQuery.Count();
                CellHistoryAllQuery = CellHistoryAllQuery.Skip((page - 1) * rows).Take(rows);
                var CellHistoryAll = CellHistoryAllQuery.ToArray()
                    .Select(c => new {
                        c.CellCode,
                        c.CellName,
