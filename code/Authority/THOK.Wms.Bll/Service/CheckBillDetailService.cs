    ...
    public class CheckBillDetailService : ServiceBase<CheckBillDetail>, ICheckBillDetailService
    {
        ...
        public bool EditDetail(string id, string status, string operater, out string strResult)
        {
            ...
            for (int i = 0; i < ids.Length - 1; i++)
            {
                ...
                detail = CheckBillDetailRepository.GetQueryable().FirstOrDefault(a => strId == a.ID);
                if (detail != null)
                {
                    try
                    {
                        ...
                        {
                            if (detail.CheckBillMaster.Status == "2" || detail.CheckBillMaster.Status == "3")
                            {
                                detail.Status = status;
                                detail.RealQuantity = quantity;
                                detail.Storage.IsLock = "0";
                                detail.CheckBillMaster.Status = "3";
                                detail.FinishTime = DateTime.Now;
                                if (detail.CheckBillMaster.CheckBillDetails.All(c => c.Status == "2"))
                                {
                                    detail.CheckBillMaster.Status = "4";
                                }
                                result = true;
                            }
                        }
        ...
        public System.Data.DataTable CheckBillDetailTable(string billNo, int page, int rows)
        {
            var query = CheckBillDetailRepository.GetQueryable()
                                    .Where(a => a.BillNo == billNo && a.Status != "2")
                                    .OrderByDescending(a => a.Status == "1")
